...
for model_size in ["1024"]:
    for fraction in ["full"]:
        ...
        for pre in ["nopre", "yespre"]:
            ...
            if len(recorded_values) != 40:
                print("WRONG LENGTH", model_name)
                15/0

            print(model_size, fraction, pre, round(statistics.mean(recorded_values),2), "+-", round(confidence_interval(recorded_values), 2))
            if pre == "yespre":
                yes_mean = statistics.mean(recorded_values)
                yes_values = recorded_values[:]
                print("Yes pre:", sorted(yes_values))
            else:
                no_mean = statistics.mean(recorded_values)
                no_values = recorded_values[:]
                print("No pre:", sorted(no_values))
