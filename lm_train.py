...
parser.add_argument("--eval_agr", help="Evaluate on agreement minimal pairs", action='store_true')
...
parser.add_argument("--eval_perplexity_difference", help="Evaluate 2 models on which tokens give the greatest perplexity difference", action='store_true')
parser.add_argument("--perplexity_difference_model_name", help="Model name for the second model", type=str, default=None)
...
if args.eval_perplexity_difference:

    if args.architecture == "GPT2":

        model2 = GPT2LM(vocab_size=vocab_size, emb_size=args.n_embd, n_positions=args.n_positions, n_layer=args.n_layer, n_head=args.n_head, dropout=args.dropout, save_dir=args.weight_dir, model_name=args.perplexity_difference_model_name).to(device)

    elif args.architecture == "LSTM":

        model2 = RNNLM(rnn_type="LSTM", vocab_size=vocab_size, emb_size=args.n_embd, hidden_size=args.n_embd, n_layers=args.n_layer, dropout=args.dropout, tie_weights=True, save_dir=args.weight_dir, model_name=args.perplexity_difference_model_name).to(device)
    ...
    perplexity_difference(trainer.model, trainer2.model, dataset.test)


if args.eval_generate:
    
    sample_generation(dataset, trainer)
    ...
    # Pure sampling
    #text = trainer.model.generate(input_ids, do_sample=True, max_length=min(args.n_positions-10,100), top_p=1.00, top_k=0, early_stopping=True, pad_token_id=dataset.tokenizer.pad_token_id)
    #text = dataset.tokenizer.decode(text[0], skip_special_tokens=True)
    #logging.info("Pure sampling:")
    #logging.info(text)

    # Top-40
    #text = trainer.model.generate(input_ids, do_sample=True, max_length=min(args.n_positions-10,100), top_p=1.00, top_k=40, early_stopping=True, pad_token_id=dataset.tokenizer.pad_token_id)
    #text = dataset.tokenizer.decode(text[0], skip_special_tokens=True)
    #logging.info("Top-40:")
    #logging.info(text)
